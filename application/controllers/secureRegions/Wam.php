...
class Wam extends Main
{
	...
	function logout()
	{
		...
		$this->session->unset_userdata('sess_current_uid');
	...
	function view_profile()
	{
		...
		if (!empty($_POST['update_password_button'])) {
			...
			if ($new_password === $re_new_password) {
				if ($this->data['user_data']->password == md5($old_password)) {
					...
					$insertStatus = $this->Common_model->update_operation(array('table' => 'admin', 'data' => $enter_data, 'condition' => "admin_id = " . $this->data['session_uid']));
		...
		if (!empty($_POST['update_username_button'])) {
			...
			if ($this->data['user_data']->password == md5($password)) {
				$is_exist = $this->Common_model->getData(array('select' => '*', 'from' => 'admin', 'where' => "username = '$username' and admin_id != " . $this->data['session_uid']));
		...
		$this->data['employee_data'] = $this->Employee_Model->get_employee(array("admin_id" => $this->data['session_uid'], 'details' => 1));
